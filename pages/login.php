...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err)){
        ...
        $sql = "SELECT user_id, username, firstname, lastname, password FROM users WHERE username = :username";
                ...
                if($stmt->rowCount() == 1){
                    if($row = $stmt->fetch()){
                        ...
                        if(password_verify($password, $hashed_password)){
                            ...
                            $read = $db->Read('usermeta', "*", '`user_id` = '.$user_id.' AND `usermeta_key` = "user_role";');
                            $array = [];
